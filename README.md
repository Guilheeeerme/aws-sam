...
```bash
PrimeiraFuncaoSAM$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
PrimeiraFuncaoSAM$ sam local start-api
PrimeiraFuncaoSAM$ curl http://localhost:3000/
```
...
```bash
PrimeiraFuncaoSAM$ sam logs -n HelloWorldFunction --stack-name PrimeiraFuncaoSAM --tail
```
...
```bash
PrimeiraFuncaoSAM$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name PrimeiraFuncaoSAM
```
